...
function activateTab(button, contentContainer, activeState) {
    ...
    const targetPanel = contentContainer.querySelector(`:scope > .tab-panel[data-id="${button.dataset.id}"]`);
...
function setActiveTabByIndex(tabsContainer, index) {
    const tabButtons = tabsContainer.querySelectorAll(':scope > .tab-list > .tab-button');
    if (tabButtons.length === 0 || index < 0 || index >= tabButtons.length) return;

    const button = tabButtons[index];

    const dataId = tabsContainer.dataset.id;
    const contentContainer = document.querySelector(`.tab-panels[data-id="${dataId}"]`);
    if (!contentContainer) return;

    let activeButton = tabsContainer.querySelector(':scope > .tab-list > .tab-button.active');
    let activePanel = contentContainer.querySelector(':scope > .tab-panel.active');
    const activeState = {
        activeButton,
        activePanel
    };
...
function initTabContentPanels(tabsContainer, contentContainer) {
    ...
    let initialButton = tabsContainer.querySelector(':scope > .tab-list > .tab-button[data-initial="true"]');
    if (initialButton) {
...
function initTabBackgroundIndicator(tabGroup) {
    const tabButtons = tabGroup.querySelectorAll(':scope > .tab-button');
    if (tabButtons.length === 0) return;
    ...
    let activeTab = tabGroup.querySelector(':scope > .tab-button.active');
...
function initTabSelectors() {
    ...
    tabContainers.forEach((container) => {
        ...
        const panelsContainer = document.querySelector(`.tab-panels[data-id="${dataId}"]`);
